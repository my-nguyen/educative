...
class NextLetter {
    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            val letterss = arrayOf(
                charArrayOf('a','c','f','h'), charArrayOf('a','c','f','h'),
                charArrayOf('a','c','f','h'), charArrayOf('a','c','f','h')
            )
            val keys = arrayOf('f', 'b', 'm', 'h')
            for (i in letterss.indices) {
                val next = NextLetter().searchNextLetter(letterss[i], keys[i])
                println("letters: ${letterss[i].contentToString()}, key: ${keys[i]}, next: $next")
            }
        }
    }
}